...
type StrSubed<A extends string, B extends string> = StrSubedSub<A, B> extends { r: infer R } ? R extends '' ? 0 : R : never
type Filled<F extends Tostrable, B, R extends string = ''> = B extends `${any}${infer B}` ? Filled<F, B, `${R}${F}`> : R
type PreAligned<A extends string, B extends string, F extends Tostrable> = `${Filled<F, StrSubed<A, B>>}${A}`
// type PostAligned<A extends string, B extends string, F extends Tostrable> = `${A}${Filled<F, StrSubed<A, B>>}`
type LeadingLess<F extends Tostrable, N extends string> = N extends `${F}${infer K}` ? K extends '' ? N : LeadingLess<F, K> : N
type StringReved<N, R extends string = ''> = N extends `${infer I}${infer S}` ? StringReved<S, `${I}${R}`> : R
type LenCmp<A extends string, B extends string> = PreAligned<A, B, 0> extends A ? PreAligned<B, A, 0> extends B ? 0 : -1 : 1
...
namespace Xcr {
	...
	export type Ex<T extends 0 | 9, N extends string> = `${T},${N}` extends '0,-1' ? '0' : `${T},${N}` extends '9,0' ? '-1' : N extends `-${infer K}` ? `-${Uns<Rev<T>, K>}` : Uns<T, N>
}
export type Xcr<T extends 0 | 9, N extends string> = Xcr.Ex<T, N>
...
namespace Aos {
	...
	type SntAosUns<T extends 0 | 9, A extends string, B extends string> = `${T},${Cmp.Ex<A, B>}` extends '9,1' ? `-${AosHal<T, B, A>}` : AosHal<T, A, B>
	export type Ex<T extends 0 | 9, A extends string, B extends string> = A extends `-${infer A}` ? B extends `-${infer B}` ? `${T extends 0 ? '-' : ''}${SntAosUns<T, B, A>}` : `${T extends 9 ? '-' : ''}${SntAosUns<Rev<T>, B, A>}` : B extends `-${infer B}` ? SntAosUns<Rev<T>, A, B> : SntAosUns<T, A, B>
}
export type Aos<T extends 0 | 9, A extends string, B extends string> = Aos.Ex<T, A, B>
...
export namespace Cmp {
	...
	export type Ex<A extends string, B extends string> = A extends `-${infer A}` ? B extends `-${infer B}` ? Uns<B, A> : 1 : B extends `-${any}` ? -1 : Uns<A, B>
...
export type Cmp<A extends string, W extends Obj, B extends string> = W[Cmp.Ex<`${A}`, `${B}`>]
...
type ISAIni<T extends 0 | 1, Z extends 0 | 1 = 1> = Z extends 0 ? ['', []][T] : ISAFmw<T, ISAIni<T, 0>, ISAIni<T, 0>>
